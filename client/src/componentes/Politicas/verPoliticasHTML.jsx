...
export const VerPoliticasHTML = (props) => {
  const {
    ...
    numeros,
    esEmpleador,
    manejoEditarPolitica // Se agrega handleEditarPolitica
  } = props;
    ...
    <div className="container">
      {cargando ? (
        <div>
          ...
          <Modal {...props} />
          <div ref={botonRef} data-bs-toggle="modal" data-bs-target={`#${modalID}`} />
          ...
          <table className="table table-hover mt-titulo">
            ...
            <tbody>
              {politicasAct.map((politica, index) => (
                <tr key={index} onClick={() => abrirModalPolitica(politica)}>
                  <td>
                    <button className="btn" onClick={() => manejoEditarPolitica(politica)}> {/* Se agrega botón de edición */}
                      {politica.titulo}
                    </button>
                  </td>
          ...
          <nav>
            <ul className="pagination">
              <li className="page-item">
                <button className="page-link" onClick={() => actualizarPagina(paginaActual > 1 ? paginaActual - 1 : 1)}>
                  <FontAwesomeIcon icon={faChevronLeft} />
                </button>
              </li>
              {numeros.map((n) => (
                <li className={`page-item ${paginaActual === n ? 'active' : ''}`} key={n}>
                  <button className="page-link" onClick={() => actualizarPagina(n)}>
                    {n}
                  </button>
                </li>
              ))}
